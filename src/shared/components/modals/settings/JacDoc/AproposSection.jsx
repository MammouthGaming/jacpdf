...
export default function AproposSection({ onOpenVersionModal }) {
  return (
    <div className="fsm-section">
      ...
      <p className="fsm-section-sub">Informations sur JacDoc.</p>

      <div className="fsm-about-card">
        <div className="fsm-logo">
          <img src={APP_LOGOS.jacdoc} alt="" className="fsm-logo-img" draggable="false" />
          <span className="fsm-logo-jac">Jac</span>
          <span className="fsm-logo-pdf">Doc</span>
          <button className="fsm-version-tag" onClick={onOpenVersionModal}>Version {getAppVersion('jacdoc')}</button>
        </div>
        <p className="fsm-about-text">
          Éditeur de documents de JacSuite avec mise en page, règles,
          export, sauvegarde, commentaires et collaboration.
        </p>
      </div>
      ...
      <p className="fsm-copyright">© 2026 JacDoc — Tous droits réservés</p>
    </div>
  )
}