...
// Phase 1 du refactor multi-apps JacSuite. Les groupes d'onglets sont
// partagés par toutes les apps via le shell global.
//
// État géré :
...
//
// Le hook reçoit en paramètre les setters/refs du shell dont il dépend.

import { useState, useEffect, useRef } from 'react'
import { tabGroupsStore } from "@/shared/stores/system/tabGroupsStore"

// Doit rester EN PHASE avec TAB_GROUP_COLORS de SuiteShell.jsx (seule la longueur
...
export function useTabGroups({ tabs, setTabs, activeId, setActiveId, tabRefs }) {
  ...
  const closeGroupTabs = (groupId) => {
    ...
    if (remaining.length === 0) {
      ...
      setTabs([{ id: newId, app: 'jacpdf', type: 'home' }])
      setActiveId(newId)
    } else {
      setTabs(remaining)
